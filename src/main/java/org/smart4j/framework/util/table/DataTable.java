...
public class DataTable {
    ...
    public void print() {
	List<String> headers = columnNames();
	List<List<String>> rows = new ArrayList<>();
	for (DataRow row : rows().asList()) {
	    rows.add(row.columnNames().stream().map(e -> String.valueOf(row.getValue(e))).collect(Collectors.toList()));
	}
	PrintUtil.printAsTable(headers, rows);
    }
